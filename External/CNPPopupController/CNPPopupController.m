    ...
    if (self) {
        ...
        if (contents) {
            for (id object in contents) {
                NSAssert(
                         [object class] != [NSAttributedString class]
                         || [object class] != [UIImage class]
                         || [object class] != [UITextField class]
                         ,@"Contents can only be of NSAttributedString or UIImage class.");
            }
        }
        if (buttonItems) {
...
- (void)setUpPopup {
    ...
    if (self.contents) {
        for (NSObject *content in self.contents) {
            ...
            else if ([content isKindOfClass:[UITextField class]]) {
                UITextField*field=[self textFieldItem:(UITextField*)content];
                //[field sizeToFit];
                [self.contentView addSubview:field];
                [self.contentView addConstraint:[NSLayoutConstraint constraintWithItem:field attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationLessThanOrEqual toItem:field attribute:NSLayoutAttributeWidth multiplier:1.0 constant:0]];
            }
        }
    }
...
- (void)actionButtonPressed:(CNPPopupButton *)sender {
    ...
    if (sender.item.selectionHandler) {
       dismiss= sender.item.selectionHandler(sender.item);
    }
    if (dismiss) {
...
- (void)dismissPopupControllerAnimated:(BOOL)flag withButtonTitle:(NSString *)title {
    ...
    if ([self.delegate respondsToSelector:@selector(popupController:willDismissWithButtonTitle:)]) {
       dismiss= [self.delegate popupController:self willDismissWithButtonTitle:title];
    }
    if (!dismiss) {
        return;
    }
...
- (UITextField *)textFieldItem:(UITextField *)item {
    //item.frame=CGRectMake(0, 0, self.contentView.frame.size.width,30);
